    ...
    switch(appData.state)
    {
        ...
        case APP_STATE_SCHEDULE_WRITE:
            ...
            else
            {
                ...
                {
                    if ((appData.readBuffer[i] >= '0') && (appData.readBuffer[i] <= '9'))
                    {
                        appData.contador += 1;
                        appData.acum = appData.acum * 10;
                        appData.acum += (appData.readBuffer[i] - '0');
                    }
                    if ((appData.contador == 2) || (appData.readBuffer[i] == 13))
                    {
                        appData.writeBuffer[0] = appData.readBuffer[i];
                        appData.writeBuffer[1] = '=';
                        appData.acum++;
                        if (appData.acum == 100) {
                            appData.writeBuffer[2] = '1';
                            appData.writeBuffer[3] = '0';
                            appData.writeBuffer[4] = '0';
                            appData.numBytesWrite = 5;
                        }
                        else if (appData.contador == 2) {
                            appData.writeBuffer[2] = '0' + appData.acum / 10;
                            appData.writeBuffer[3] = '0' + appData.acum % 10;
                            appData.numBytesWrite = 4;
                        }
                        else {
                            appData.writeBuffer[2] = '0' + appData.acum;
                            appData.numBytesWrite = 3;
                        }
                        USB_DEVICE_CDC_Write(USB_DEVICE_CDC_INDEX_0,
                                &appData.writeTransferHandle,
                                appData.writeBuffer, appData.numBytesWrite,
                                USB_DEVICE_CDC_TRANSFER_FLAGS_DATA_COMPLETE);
                        appData.contador = 0;
                        appData.acum = 0;
                    }
